...
def _check_inputs(X, ql=0.98, p_cross=None):
	"""
...
def _calc_delta(X, tt, n_samples, p=2, axis=1, p_cross=None, vars=[(0,0)],\
	...
	if comm is None:
		...
		for vv in range(n_vars):
			delta_self[:,vv] = np.sum(np.abs(X[tt,:,vv] - X[:,:,vv])**p,
											axis=axis)**(1/p)
		...
		norm_delta_self = np.zeros([n_vars,1])
		for vv in range(n_vars):
			sum_abs_dist_p = np.sum(np.abs(X[tt,:,vv] - X[:,:,vv])**p, axis=1)
			...
			comm.Reduce(sum_abs_dist_p, \
						sum_abs_dist_p_reduced, op = MPI.SUM, root = 0)
...
def _compute_distributed_time(
	X, ql=0.98, p=2, theta_fit="sueveges",
	...
	## check data format
	X, axis, vars, dim_out, ql = _check_inputs(X, ql=ql, p_cross=p_cross)
	...
	perrank = n_samples // size
	remaind = n_samples % size
	comm.Barrier()
	...
	if rank != size - 1:
		...
		for tt in range(rank * perrank, (rank + 1) * perrank):
			...
			res = time_parallel_loop(
				X=X, tt=tt, tt_local=tt_local, vars=vars, n_vars=n_vars,
		...
		for tt in range((size-1)*perrank, n_samples):
			pbar.update(1)
	...
	if rank == 0:
		...
		exceeds = np.concatenate(exceeds, axis=0)
		if p_value is not None:
...
def _compute_serial_time(
	X, ql=0.98, p=2, theta_fit="sueveges", p_value=None,
	dql=None, exp_test='anderson', p_cross=None, comm=None, **kwargs):
	"""
	ql: float between 0 and 1 [default: 0.98]
		Quantile threshold to determine the exceeds.
	p_value: float between 0 and 1 or None [default: None]
	...
	## MPI communicator `comm` must be provided for distributed data
	if comm is not None:
	...
	## initialize vectors
	max_n_exceeds = np.max(n_exceeds)
	d       = np.zeros([n_samples,dim_out], dtype=float_type)
	theta   = np.zeros([n_samples,dim_out], dtype=float_type)
	H0      = np.zeros([n_samples,dim_out], dtype=bool)
	exceeds = \
		np.zeros([n_samples,max_n_exceeds,dim_out], dtype=float_type)*np.nan
	idx_exceeds = \
		np.zeros([n_samples,max_n_exceeds,dim_out], dtype=float_type)*np.nan
	ql_opt = np.zeros([n_samples,dim_out], dtype=float_type)
	if p_cross is not None:
		alpha = np.ones([n_samples,dim_out], dtype=float_type)

	## loop over time snapshots (epochs)
	for tt in tqdm(range(n_samples), desc='# iterations on epochs (serial)'):
		res = _calc_epoch(
			X=X,
			tt=tt,
			vars=vars,
...
def compute(
	X, ql=0.98, p=2, theta_fit="sueveges",
	p_value=None, dql=None, exp_test='anderson',
	p_cross=None, distributed='none',
	comm=None, **kwargs):
	"""
	compute local indices using:
		- distributed = 'none' (serial implementation)
	"""

	## switch between distributed vs serial input data
	if distributed.lower() == 'none':
		results, comm = _compute_serial_time(
			X=X,
			ql=ql,
			p=p,
			theta_fit=theta_fit,
		...
		results, comm = _compute_serial_time(
			...
			comm=comm)
		return results, comm
	elif distributed.lower() == 'time':
		results, comm = _compute_distributed_time(
			X=X,
			ql=ql,
			p=p,
			theta_fit=theta_fit,
			...
			p_cross=p_cross)
		return results, comm
	elif distributed.lower() == 'time-data':
		#todo
		pass
