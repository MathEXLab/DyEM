...
def _calc_epoch(
	...
	n_samples = n_samples -1	# exclude the last one, which is the new data
...
def _calc_delta(X, tt, n_samples, p=2, axis=1, p_cross=None, vars=[(0,0)],\
	...
	if comm is None:
		...
		for vv in range(n_vars):
			delta_self[:,vv] = np.sum(np.abs(X[tt,:,vv] - X[:-1,:,vv])**p,	# exclude the last one, which is the new data
											axis=axis)**(1/p)
		...
		norm_delta_self = np.zeros([n_vars,1])
		for vv in range(n_vars):
			sum_abs_dist_p = np.sum(np.abs(X[tt,:,vv] - X[:-1,:,vv])**p, axis=1)	# exclude the last one, which is the new data
...
def compute(
	...
	n_samples = n_samples + 1	# add the new data
	...
	for tt in range(pred_length):
		...
		res = _calc_epoch(
			X=temp,
			tt=n_samples-1,	# only loop over the appended new data
			# tt = tt,
			vars=vars,
			axis=axis,
			p=p, ql=ql,
